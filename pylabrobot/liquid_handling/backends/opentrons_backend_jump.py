...
# 这里是不是得考虑一下
  ...
  async def drop_tips(self, ops: List[Drop], use_channels: List[int]):
    ...
    #print(f"Dropping tip to: {'trash' if use_fixed_trash else op.resource.name}")
    offset_x, offset_y, offset_z = (
      op.offset.x,
      op.offset.y,
      op.offset.z,
  ...
  async def aspirate(self, ops: List[SingleChannelAspiration], use_channels: List[int],**kwargs):
    ...
    if self.simulation:
      ...
      if op.blow_out_air_volume: # this is for air
        ot_api.lh.aspirate(
          labware_id,
          well_name=op.resource.name,
          pipette_id=pipette_id,
          volume=op.blow_out_air_volume,
          flow_rate=flow_rate,
          offset_x=offset_x,
          offset_y=offset_y,
          offset_z=offset_z + 5,  # Slightly higher for air intake
        )
...
# In this way, we can use the same function for both aspirate and dispense and make use of all of the information
